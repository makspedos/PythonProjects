...
def generate_pdf(request):
    ...
    last_purchase = Orders.objects.all()
    last_purchase = last_purchase[len(last_purchase)-1]
    ...
    p.drawString(100, 100,
                 f'Last purchase: {last_purchase.customer.name}')
    p.showPage()
    p.save()
...
def checkout(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            order = Orders.objects.create(customer=customer, status_id=2, delivery_type=delivery_type, payment_type=payment_type #total_price=price_for_order
            )
            for item, value in data.items():
                if item.startswith('number_in_basket_'):
                    ...
                    product.count = value
                    product.order = order
                    product.save(force_update=True, is_active=False)
...
def basket(request):
    ...
    product_in_basket = Basket.objects.filter(is_active=True)
    if is_delete == 'true':
    ...
    products_in_basket = Basket.objects.filter(session_key=session_key, is_active=True, order__isnull=True)
